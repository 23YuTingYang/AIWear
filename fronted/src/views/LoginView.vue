 ...
 * 登录/注册页（学生演示项目）
 *
 * 支持验证码登录与密码登录；左侧轮播图，右侧表单。登录成功后写入登录状态，再跳转目标页面。
 * 要点：切换登录方式时清空表单并清除校验；重定向参数来自路由查询，用于登录后跳回原页面。
 */
...
import logoMark from '../assets/image/logo-yt-mark.svg'
import logoFull from '../assets/image/logo-yt-full.svg'
...
<template>
  <div class="login">
    <a href="/" class="login-top-logo" aria-label="YUTING YANG">
      <img :src="logoMark" alt="YUTING YANG" class="login-top-logo-img" />
    </a>
      ...
      <section class="panel">
        <header class="brand">
          <div class="brand-logo">
            <img :src="logoFull" alt="YUTING YANG" class="brand-logo-icon" />
          </div>
