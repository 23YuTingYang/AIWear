 ...
 * HTTP 请求封装（学生演示项目）
 *
 * - 基础地址为 '/api'，开发时由 Vite 代理到 .env.development 中的代理目标，避免跨域。
 * - 请求拦截器：从登录状态中取令牌，自动附加认证请求头，需登录的接口无需在业务里手写。
 * - 响应成功但业务码不是 200：视为业务错误，抛出带提示信息的异常，并挂上响应对象便于页面取后端信息。
 * - 响应失败（网络错误、4xx/5xx）：统一补全错误提示，再交给页面用 ElMessage 展示。
 */
